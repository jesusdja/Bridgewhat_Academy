  ...
  Widget containerQuestions({required List<String> colum1, required int type}){
    ...
    for (int x = 0; x < colum1.length; x++) {
      ...
      listW.add(
        InkWell(
          child: Container(
            padding: EdgeInsets.symmetric(horizontal: sizeW * 0.05,vertical: sizeH * 0.02),
            ...
            child: Text(colum1[x],style: AcademyStyles().stylePoppins(size: sizeH * 0.02,color: Colors.black)),
          ),
          onTap: (){
            if(type == 0){
              if(mapColor.containsKey(colum1[x])){
